...
HTTP의 Connectionless, Stateless 특성 때문에 세션을 이용한 서버 인증 방식을 주로 사용한다.

하지만 저장해야 하는 세션의 수가 많아진다면 서버에 과부하가 발생하게 될 수 있다.

토큰 인증 방식은 서버측에 데이터를 담아두지 않는다.

서버에서 발급받은 AccessToken을 클라이언트사이드에 저장하며, 서버로 보내는 HTTP의 요청 헤더의 Authorization 값에 토큰을 포함시켜 전달한다.
...
JWT는 토큰의 만료 기한을 설정 할 수 있다.

만료 기한을 짧게 설정하면 토큰이 탈취되더라도 빠르게 만료되어 보안성은 높아지지만 자주 로그인해야 하며 글을 작성하는 등의 행위 중에 토큰이 만료될 수 있다.

반면 만료 기한을 길게 설정하면 토큰이 탈취되었을 때 만료되기 전까지 제약이 없어 보안성이 떨어진다.
